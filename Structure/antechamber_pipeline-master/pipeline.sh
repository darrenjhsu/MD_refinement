...
do
	...
	python3 fix_atom_numbering.py "$temp2"_H.mol2		# fix atom names by adding numbers to each atom name; no potential for a bug because mol2 file are fairly flexible
	...
	sed -e 's,aaa,'"$temp2"'_H_bcc,g' < leap_input.in > temp.in	# sed replace some search strings and output a temporary tleap input file
